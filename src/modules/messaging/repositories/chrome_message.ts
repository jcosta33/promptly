 ...
 */
export function listen_for_messages<T = unknown, R = any>(
  handler: (message: T, sender: chrome.runtime.MessageSender) => R | Promise<R>,
): () => void {
  const listener = (
    message: T,
        ...
        result
          .then((response) => {
            try {
              ...
              logger.warn(
                "Error sending response (might be normal if receiver disconnected):",
                error,
              );
            }
          })
